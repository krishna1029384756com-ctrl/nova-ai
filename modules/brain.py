...
class Brain:
    ...
    def _think(self, message):
        try:
            ...
            # Ollama not installed, not running, or timed out - fall back
            # to basic replies rather than leaving NOVA silent.
            print(f"[AI] Ollama unavailable, using basic replies instead ({e})")
            return self._converse(message.lower().strip())

    def _converse(self, message):
        # Fallback rule-based replies, used only when the local AI model
        # (Ollama) isn't reachable.
        if "who are you" in message or "your name" in message:
            return "I'm NOVA. I'm in basic mode right now - start Ollama for smarter replies."
        ...
        return f'I heard: "{message}" — I don\'t have a specific response for that (Ollama seems offline).'
