...
class MainActivity : ComponentActivity() {
    ...
    private val textColor = Color.rgb(242, 242, 248)
    ...
    private fun buildUi() {
        ...
        titleBox.addView(
            textView("NOVA Companion", 20f, textColor, true)
        )

        titleBox.addView(
            textView("Phone control bridge", 13f, muted)
        )
        ...
        connectionCard.addView(
            textView("Connection", 16f, textColor, true)
        )
        ...
        connectButton =
            textView("START CONNECTION", 14f, Color.WHITE, true).apply {
                gravity = Gravity.CENTER
                ...
                setOnClickListener {
                    // Milestone 0:
                    // UI only. LAN discovery comes next.
        ...
        deviceCard.addView(
            textView("This device", 16f, textColor, true)
        )

        deviceCard.addView(
            textView("Android Companion", 14f, muted).apply {
                setPadding(0, dp(10), 0, dp(3))
            }
        )

        deviceCard.addView(
            textView("Waiting for NOVA on your PC", 13f, muted)
        )
        ...
        protocolCard.addView(
            textView("Current milestone", 16f, textColor, true)
        )

        protocolCard.addView(
            textView(
                "LAN discovery  →  pair  →  PING / PONG",
                14f,
                muted
            ).apply {
        ...
        protocolCard.addView(
            textView(
                "No AI • No file transfer • No always-on service",
                12f,
                muted
            ).apply {
